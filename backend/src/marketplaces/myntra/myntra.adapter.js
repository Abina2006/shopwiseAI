 ...
 * Handles URL parsing, Myntra style ID extraction, direct product URL verification,
...
export class MyntraAdapter {
  constructor() {
    this.marketplace = 'Myntra';
    this.baseUrl = 'https://www.myntra.com';
  }

  parseUrl(url = '') {
    if (!url || typeof url !== 'string') {
      return { isDirectUrl: false, styleId: null, urlType: 'INVALID' };
    }

    const cleanUrl = url.trim();
    const idMatch = cleanUrl.match(/\/([0-9]{5,12})\/buy/i) || cleanUrl.match(/\/style\/([0-9]{5,12})/i);
    const styleId = idMatch ? idMatch[1] : null;

    const isSearchUrl = cleanUrl.includes('/search') || cleanUrl.includes('f=') || cleanUrl.includes('q=');
    const isDirectUrl = Boolean(styleId) && !isSearchUrl;
  ...
  normalizeProduct(raw = {}) {
    ...
    const directUrl = raw.productUrl || (raw.styleId ? `${this.baseUrl}/${raw.styleId}/buy` : null);
    const isDirect = this.parseUrl(directUrl).isDirectUrl;

    return {
      marketplace: this.marketplace,
      externalProductId: raw.styleId || raw.externalProductId || null,
      title: raw.title || raw.name || 'Myntra Product',
      brand: raw.brand || (raw.title ? raw.title.split(' ')[0] : 'Myntra'),
      model: specs.modelKey || raw.model || null,
      variant: [specs.storage, specs.size, specs.color].filter(Boolean).join(' ') || raw.variant || null,
  ...
  async searchProducts(query = '') {
    ...
    try {
      ...
      const listings = await prisma.productListing.findMany({
        where: {
          sellerName: { contains: 'Myntra', mode: 'insensitive' },
          product: {
            OR: [
      ...
      return listings.map(l => {
        const { styleId } = this.parseUrl(l.sellerUrl);
        return this.normalizeProduct({
          styleId: styleId || l.model || null,
      ...
      console.warn(`[MyntraAdapter] searchProducts error: ${err.message}`);
      return [];
    }
  }

  async getProduct(productIdOrUrl = '') {
    const { styleId } = this.parseUrl(productIdOrUrl);
    const identifier = styleId || productIdOrUrl;
    ...
    try {
      ...
      console.warn(`[MyntraAdapter] getProduct error: ${err.message}`);
    }
  ...
  async verifyListing({ productName, targetUrl, storedPrice = null }) {
    const { isDirectUrl, styleId } = this.parseUrl(targetUrl);
    ...
    if (!isDirectUrl) {
      return {
        ...
        disclaimer: 'This URL is a search page containing multiple products. Only direct product links (/STYLE_ID/buy) are eligible for verified pricing.'
      };
    }
...
export default new MyntraAdapter();
