/**
 * Croma Marketplace Adapter
 * Handles URL parsing, Croma product code extraction, direct product URL verification,
...
export class CromaAdapter {
  constructor() {
    this.marketplace = 'Croma';
    this.baseUrl = 'https://www.croma.com';
  }

  parseUrl(url = '') {
    if (!url || typeof url !== 'string') {
      return { isDirectUrl: false, cromaId: null, urlType: 'INVALID' };
    }

    const cleanUrl = url.trim();
    const idMatch = cleanUrl.match(/\/p\/([0-9]{5,12})/i);
    const cromaId = idMatch ? idMatch[1] : null;

    const isSearchUrl = cleanUrl.includes('/search') || cleanUrl.includes('q=');
    const isDirectUrl = Boolean(cromaId) && !isSearchUrl;
  ...
  normalizeProduct(raw = {}) {
    ...
    const directUrl = raw.productUrl || (raw.cromaId ? `${this.baseUrl}/p/${raw.cromaId}` : null);
    const isDirect = this.parseUrl(directUrl).isDirectUrl;

    return {
      marketplace: this.marketplace,
      externalProductId: raw.cromaId || raw.externalProductId || null,
      title: raw.title || raw.name || 'Croma Product',
      brand: raw.brand || (raw.title ? raw.title.split(' ')[0] : 'Croma'),
  ...
  async searchProducts(query = '') {
    ...
    try {
      ...
      return listings.map(l => {
        const { cromaId } = this.parseUrl(l.sellerUrl);
        return this.normalizeProduct({
          cromaId: cromaId || l.model || null,
      ...
      console.warn(`[CromaAdapter] searchProducts error: ${err.message}`);
      return [];
    }
  }

  async getProduct(productIdOrUrl = '') {
    const { cromaId } = this.parseUrl(productIdOrUrl);
    const identifier = cromaId || productIdOrUrl;
    ...
    try {
      ...
      console.warn(`[CromaAdapter] getProduct error: ${err.message}`);
    }
  ...
  async verifyListing({ productName, targetUrl, storedPrice = null }) {
    const { isDirectUrl, cromaId } = this.parseUrl(targetUrl);
    ...
    if (!isDirectUrl) {
      return {
        ...
        disclaimer: 'This URL is a search page containing multiple products. Only direct product links (/p/PID) are eligible for verified pricing.'
      };
    }
...
export default new CromaAdapter();
